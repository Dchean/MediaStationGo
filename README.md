...
默认部署现在改成了**新手极简模式**：只需要准备一个 `docker-compose.yml`，再在 `.env` 里填写两个目录。
...
| 变量 | 填什么 |
| --- | --- |
| `MEDIASTATION_MEDIA_DIR` | NAS/服务器上的真实媒体目录，例如 `/volume1/media`、`/mnt/media`、`/vol1/1000/Docker/moviepilot-v2/media` |
| `MEDIASTATION_DOWNLOAD_DIR` | NAS/服务器上的真实下载目录，例如 `/volume1/downloads`、`/mnt/downloads`、`/vol1/1000/qBittorrent/downloads` |
...
默认文件只保留新手必需项。需要直读宿主机原始路径、Telegram 代理、硬件加速、更多转码环境变量时，参考仓库中的高级示例：
...
生产环境可以固定镜像版本，避免 `latest` 自动变化。只需要在 `.env` 里增加：
...
项目默认内置极简 `docker-compose.yml`，新手通常只需要下面几个变量：
